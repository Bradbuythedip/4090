...
def release_gpu(gpu_data):
    ...
    if gpu_data and 'context' in gpu_data:
        try:
            ...
            # Try a more aggressive cleanup on error
            try:
                while cuda.Context.get_current() is not None:
                    cuda.Context.pop()
            except:
...
def multi_gpu_search():
    ...
    global RUNNING, SOLUTION_FOUND, START_TIME, KEYS_CHECKED, BEST_GLOBAL_MATCHES
...
if __name__ == "__main__":
    try:
        ...
        # Ensure all CUDA contexts are cleaned up
        try:
            cuda_contexts_cleaned = 0
            while cuda.Context.get_current() is not None:
                cuda.Context.pop()
                cuda_contexts_cleaned += 1
            if cuda_contexts_cleaned > 0:
                print(f"Cleaned {cuda_contexts_cleaned} CUDA contexts after search")
        except Exception as e:
            print(f"Warning: {e}")
